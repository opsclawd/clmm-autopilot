...
| M6 shell UX | partial | Web/mobile shell flows and UI state are implemented; full runtime behavior is constrained by deferred Jupiter/receipt flags. |
...
| M12 devnet e2e harness | partial | Harness + runbook + receipt verification logic exist; runtime behavior remains partially deferred by Jupiter/receipt flags. |
...
- Keep `JUPITER_SWAP_DISABLED_FOR_TESTING=true`.
